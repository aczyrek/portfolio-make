...
export const projects = [
  {
    ...
    challenge:
      'Scattered tasks with no consistent prioritization — forcing daily manual "triage".',
    solution:
  ...
  },
  {
    ...
    challenge:
      "Tedious manual logging of daily Ad results from Ads Manager into spreadsheets.",
    solution:
    ...
    result:
      "Real-time analytical data for budget decisions with zero manual input.",
  ...
  },
  {
    ...
    challenge:
      "Tasks scattered across multiple boards causing prioritization and WIP tracking gaps.",
    solution:
  ...
  },
  {
    ...
    image: "/images/analytics_thumbnail_2.png",
    challenge:
      "Manual separation of food and beverage revenue from raw POS transaction data.",
    solution:
    ...
    result:
      "Zero-touch daily financial reports with automatic Kitchen / Bar revenue split.",
  ...
  },
  {
    ...
    title: "Automated Booksy Personal Reviews Scraper",
    description:
      "System that monitors a Booksy salon profile for new reviews of a specific staff member, checks them against a historical database, and logs new entries to Google Sheets.",
    tags: ["Make.com", "Web Scraping", "Data Store", "Google Sheets"],
    image: "/images/automation_thumbnail.png",
    challenge:
      "Manually checking the Booksy profile to track personal review ratings and feedback.",
    solution:
      "Scheduled Make HTTP requests scrape the Booksy reviews page, RegExp extracts the review data, and new reviews (not in Datastore) are appended to a tracking Google Sheet.",
    result:
      "Automated personal feedback tracking directly in a spreadsheet without manual browsing.",
    blueprint: "/blueprints/project-5.json",
    filename: "Booksy Personal Review.blueprint.json",
    schema: [
      { icon: "🌐", name: "HTTP Request", desc: "Scrape Booksy Page" },
      { icon: "🔍", name: "RegExp Parser", desc: "Extract Reviews" },
      { icon: "🗄️", name: "Data Store", desc: "Check if Exists" },
      { icon: "🔀", name: "Router", desc: "Filter New Reviews" },
      { icon: "📊", name: "Google Sheets", desc: "Log New Review" },
    ],
  },
  {
    id: 6,
    ...
    title: "Amplemarket to Attio CRM Sync",
    description:
      "Seamless webhook integration bridging Amplemarket email sequences and Attio CRM. It dynamically assigns sales ownership based on email data and creates Deal, Company, and Person records automatically.",
    tags: ["Make.com", "Amplemarket", "Attio", "JavaScript", "Webhooks"],
    image: "/images/analytics_thumbnail_2.png",
    challenge:
      "Manual transfer of interested leads from Amplemarket email campaigns into the Attio sales pipeline, causing delays in follow-ups.",
    solution:
      'Webhook triggers on new Amplemarket replies, filters for "Interested" tags, runs custom JavaScript to match lead ownership, and uses Attio APIs to upsert objects and create deals.',
    result:
      "Zero-touch sales handoff from email sequence responses directly to structured CRM deals.",
    blueprint: "/blueprints/project-6.json",
    filename: "Legia AmpleMarket -- Attio Deal.blueprint.json",
    schema: [
      { icon: "🪝", name: "Webhook", desc: "Listen for Replies" },
      { icon: "🧑‍💻", name: "JS Code", desc: "Match Representative" },
      { icon: "🏢", name: "Attio API", desc: "Upsert Company" },
      { icon: "👤", name: "Attio API", desc: "Upsert Person" },
